...
进程是一个独立的资源分配单元，不同进程(通常指用户进程)之间的资源是独立的，所以不能在一个进程中直接访问另一个进程的资源。但是，不同的进程又需要进行信息的交互和状态的传递等，因此需要进程间通信(IPC: Inter Process Communication)。进程间通信的方式主要有：
- 管道
- 内存映射
- 信号
- 共享内存
...
其中只有共享内存是直接通信方式(不需要经过内核，两个进程间就能直接通信)，其余方式都是间接通信方式(需要通过系统调用和内核的中转来相互传递消息)。
